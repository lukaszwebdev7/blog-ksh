...
function ProstaPostsPage(props) {
	const [ number, setNumber ] = useState(1);
	const posts = props.posts;

	const filteredArray = posts.filter((post) => post.category === 'prosta_spolka_akcyjna');
	...
	useEffect(
		() => {
			const stickyNumber = window.sessionStorage.getItem('numberOfArrayProsta');
			const stickyArray = window.sessionStorage.getItem('arrayProsta');
	...
	useEffect(
		() => {
			window.sessionStorage.setItem('numberOfArrayProsta', number),
				window.sessionStorage.setItem('arrayProsta', JSON.stringify(chosenArray));
		},
		[ 'numberOfArrayProsta', number ],
		[ 'arrayProsta', chosenArray ]
	);
...
export default ProstaPostsPage;
