...
function JawnaPostsPage(props) {
	...
	useEffect(
		() => {
			const stickyNumber = window.sessionStorage.getItem('numberOfArrayJawna');
			const stickyArray = window.sessionStorage.getItem('arrayJawna');
	...
	useEffect(
		() => {
			window.sessionStorage.setItem('numberOfArrayJawna', number),
				window.sessionStorage.setItem('arrayJawna', JSON.stringify(chosenArray));
		},
		[ 'numberOfArrayJawna', number ],
		[ 'arrayJawna', chosenArray ]
	);
